...
function init()
{
	...
	firstTime = localStorage ? (localStorage.getItem('first') == null) : true;
	tour = new Tour(GALAXY_TOUR);
	rtparam = { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBFormat, stencilBufer: false };
...
/*	canvas.addEventListener('click', function(event) {
	var x = event.pageX,
		y = event.pageY;
		ehCClick(x,y);});
	//window.addEventListener("resize", ehResize);
	//window.addEventListener("click", ehClick); */
	$( "#maindiv" ).tabs();
	$( "#maindiv" ).draggable();
	$( "#maindiv" ).resizable();
...
function fetchSetup() {
	$.ajax({
		type: "GET",
		url: "data/setuptext.txt",
		dataType: "text",
		success: function(data) {
			$( "#divSetup" ).html(data);
			applyStylesToSetupTab();
		},
		error: function(data, status, error) {
			console.log(data);
			console.log(status);
...
function applyStylesToSetupTab()
{
		$( "#divSetupIn" ).accordion( {heightStyle: "content", icons: { "header": "ui-icon-plus", "activeHeader": "ui-icon-minus" }});
	  $( "#divWelcome" ).accordion( {heightStyle: "content", icons: { "header": "ui-icon-plus", "activeHeader": "ui-icon-minus" }});
	  $( "#empireEnvironment").selectmenu();
	  $( "#empireEconomics").selectmenu();
	  $( "#empireAggression").selectmenu();
	  $( "#empireProduction").selectmenu();
	  $( "#dfltFleetFormation").selectmenu();
	  $( "#dfltFleetDisp").selectmenu();
	  $( "#dfltFleetDist").selectmenu();
	  $( "#dfltRetreatThreshold").selectmenu();
	  $("#btnSelFlag").button().click(function( event ) {flagDialog();});
	  $('input').addClass("ui-widget ui-widget-content ui-corner-all");
	  $('textarea').addClass("ui-widget ui-widget-content ui-corner-all");
	  
}
function flagSelect(selectedflag,flimage)
  {
  				$("#imgSelectedFlag").attr("src","../images/flags/" + flimage);
				$("#imgSelectedFlag").data(selectedflag);
				$( "#flagDlg" ).dialog('close');
  }
  function flagDialog()
  {
	      blnDlgOpen = true;
	      ...
	      flagcounter = 1;
			...
			strText = "<div id=\"divFlagSelect\"><table id=\"tblFlagSelect\">";
			
			for(var i=1;i<17;i++)
			{
				strText += "<tr class=\"flagTR\">";
				for(var j=1;j<16;j++)
				{
					strText += "<td class=\"flagTD\">";		
					strText += "<img id=\"eFlag" + flagcounter + "\" src=../images/flags/flag" + 
								flagcounter + ".png class=\"flagImgCl\"  onclick=\"flagSelect('eFlag" + flagcounter+"','flag"+flagcounter+".png')\"></img>";
					strText += "</td>";	
					flagcounter++;	
				}
				strText += "</tr>\r\n";
  ...
  function starSelDialog(pickResult)
  {
  		var index;
		var strText;
		...
		{
			blnDlgOpen = true;
			index = pickResult.pickedMesh.id;
			strText = "<p><b>Star id: <b>" +  hygData.stars[index].id + "<p>";
			strText += "<p><b>Star Name: <b>" +  hygData.stars[index].proper + "<p>";
			strText += "<p><b>Dist (parsec): <b>" +  hygData.stars[index].dist + "<p>";
			strText += "<p><b>Spectra: <b>" +  hygData.stars[index].spect + "<p>";
			strText += "<p><b>X: <b>" +  hygData.stars[index].x + "<p>";
			strText += "<p><b>Y" +  hygData.stars[index].y + "<p>";
			strText += "<p><b>Z: <b>" +  hygData.stars[index].z + "<p>";
			strText += "<p><b>Luminance: <b>" +  hygData.stars[index].lum + "<p>";
			strText += "<p></p>";
			strText += "<p><b><h4>[ ESC ] to exit <b><h4></p>";
			
			$( "#starDlg" ).html(strText);
			$( "#starDlg" ).dialog({
										maxWidth:200,
        								maxHeight: 300,
        								width: 150,
										...
										buttons: {
										Rezoom: function() {
														reZoom(hygData.stars[index].x,hygData.stars[index].y,hygData.stars[index].z);
														$( this ).dialog( "close" );
														}
...
var postShadersLoaded = function(){
	if( enableDataStar ){
		loadStarData( "data/stars_1000.json", function(loadedData){
...
function initScene() {
	...
	camera.position.target = { x: 0, z: 1000, pz: 500 };
	...
	$(document).ready(function() { document.getElementById('bgmusicA').play();});
	*/
}
